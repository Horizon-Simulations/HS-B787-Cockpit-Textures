...
CDN_DIR=${1:-"addons/hs-b787-cockpit-texture/development"}
...
upload () {
    ...
    counter=0
    until curl --fail -X PUT -H "X-FBW-Access-Key: $CLOUDFLARE_WORKER_PASSWORD" -T "$1" "$DEST"
    do
        sleep 1
...
for FILE in "${LOCAL_DIR}"/*; do
    ...
    echo "Purge URL: $DEST"
    curl -X POST -H "X-FBW-Access-Key: $CLOUDFLARE_WORKER_PASSWORD" -H "Content-Length: 0" "$DEST"
done